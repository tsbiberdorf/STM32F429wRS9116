...
volatile uint8_t    ix, conn_done, conn_update_done, conn_update_master_done,num_of_conn_masters = 0, num_of_conn_slaves = 0;
...
void rsi_gatt_add_attribute_to_list(rsi_ble_t *p_val, uint16_t handle,
		uint16_t data_len, uint8_t *data, uuid_t uuid, uint8_t char_prop) {
...
static uint32_t rsi_ble_add_simple_chat_serv(void) {
	...
	rsi_ble_add_char_val_att(new_serv_resp.serv_handler,
			new_serv_resp.start_handle + 2, new_uuid,
			RSI_BLE_ATT_PROPERTY_READ | RSI_BLE_ATT_PROPERTY_NOTIFY | RSI_BLE_ATT_PROPERTY_WRITE, data,
			sizeof(data), 1);
...
static uint32_t rsi_ble_add_simple_chat_serv3(void) {
	...
	static const uuid_t custom_service =
	{
			.size = 16,
...
static uint32_t rsi_ble_add_custom_service_serv(void) {
	...
	rsi_ble_add_char_val_att(new_serv_resp.serv_handler,
			new_serv_resp.start_handle + 2, new_uuid,
			RSI_BLE_ATT_PROPERTY_READ | RSI_BLE_ATT_PROPERTY_INDICATE, data,
			sizeof(data), 1);
	...
	rsi_ble_add_char_val_att(new_serv_resp.serv_handler,
			...
			RSI_BLE_ATT_PROPERTY_READ, format_data, sizeof(format_data), 1);
...
void rsi_ble_simple_peripheral_on_remote_features_event(
	...
	//! convert to ascii
	rsi_6byte_dev_address_to_ascii(remote_dev_addr_conn,
			rsi_ble_event_remote_features->dev_addr);
		...
		rsi_ble_event_adv_report_t *adv_report)
{
	...
	rsi_6byte_dev_address_to_ascii(remote_dev_addr,
...
void rsi_ble_on_enhance_conn_status_event(
	...
	if(remote_device_role == SLAVE_ROLE)						//check for the connection is from slave or master
	{
		ble_conn_id = rsi_get_ble_conn_id(remote_dev_addr_conn, NULL, 0);
		//! copy to conn specific buffer
		memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_enhc_conn_status, resp_enh_conn, sizeof(rsi_ble_event_enhance_conn_status_t));
...
static void ble_on_conn_update_complete_event(
	...
	else
	{
		...
		//! copy to conn specific buffer
		memcpy(&rsi_ble_conn_info[ble_conn_id].conn_update_resp,
				resp_conn_update,
 ...
 * This callback function indicates the status of the connection
 */
static void rsi_ble_on_remote_conn_params_request_event(rsi_ble_event_remote_conn_param_req_t *remote_conn_param,uint16_t status)
{
	//! convert to ascii
	rsi_6byte_dev_address_to_ascii(remote_dev_addr_conn,
			remote_conn_param->dev_addr);
	//! get conn_id
	ble_conn_id = rsi_get_ble_conn_id(remote_dev_addr_conn, NULL, 0);

	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_app_remote_device_conn_params,
			remote_conn_param, sizeof(rsi_ble_event_remote_conn_param_req_t));
...
static void rsi_ble_on_disconnect_event(
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_ble_disconn_resp,
			resp_disconnect, sizeof(rsi_ble_event_disconnect_t));
...
static void rsi_ble_on_event_write_resp(uint16_t event_status, rsi_ble_set_att_resp_t *rsi_ble_event_set_att_rs) {
	...
	memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_ble_write_resp_event,
			rsi_ble_event_set_att_rs, sizeof(rsi_ble_set_att_resp_t));
...
static void rsi_ble_on_gatt_write_event(uint16_t event_id,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].app_ble_write_event,
			rsi_ble_write, sizeof(rsi_ble_event_write_t));
...
static void rsi_ble_on_gatt_prepare_write_event(uint16_t event_id,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].app_ble_prepared_write_event, rsi_app_ble_prepared_write_event, sizeof(rsi_ble_event_prepare_write_t));
 ...
 * This callback function is invoked when write/notify/indication events are received
 */
static void rsi_ble_on_execute_write_event(uint16_t event_id,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].app_ble_execute_write_event, rsi_app_ble_execute_write_event, sizeof(rsi_ble_execute_write_t));
...
static void rsi_ble_on_read_req_event(uint16_t event_id,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].app_ble_read_event,
			rsi_ble_read_req, sizeof(rsi_ble_read_req_t));
...
static void rsi_ble_on_mtu_event(rsi_ble_event_mtu_t *rsi_ble_mtu) {
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].app_ble_mtu_event,
			rsi_ble_mtu, sizeof(rsi_ble_event_mtu_t));
...
static void rsi_ble_profiles_list_event(uint16_t resp_status,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].get_allprofiles, rsi_ble_event_profiles, sizeof(rsi_ble_event_profiles_list_t));
...
static void rsi_ble_char_services_event(uint16_t resp_status,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].get_char_services, rsi_ble_event_char_services, sizeof(rsi_ble_event_read_by_type1_t));
...
static void rsi_ble_on_read_resp_event(uint16_t event_status, rsi_ble_event_att_value_t *rsi_ble_event_att_val)
{
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_char_descriptors, rsi_ble_event_att_val, sizeof(rsi_ble_event_att_value_t));
 ...
 * Note: We have to send SMP passkey command
 */
static void rsi_ble_on_smp_passkey_display(
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_ble_smp_passkey_display, smp_passkey_display, sizeof(rsi_bt_event_smp_passkey_display_t));
 ...
 */
static void rsi_ble_on_smp_failed(uint16_t status,
...
static void rsi_ble_on_encrypt_started(uint16_t status,
	...
	//! copy to conn specific buffer
	memcpy(&rsi_ble_conn_info[ble_conn_id].rsi_encryption_enabled, enc_enabled, sizeof(rsi_bt_event_encryption_enabled_t));
...
void rsi_ble_on_sc_method(rsi_bt_event_sc_method_t *scmethod) {
	...
	{
		LOG_PRINT("\nOur Security method is Passkey_Entry, hence same 6 digit Numeric value to be entered on both devices using keyboard, do not enter the numeric value displayed on one device into another device using keyboard \r\n");
	}
	else{}
}
#endif
...
static int32_t rsi_ble_dual_role(void) {
	...
	uint8_t local_irk[16] = {0x4d, 0xd7, 0xbd, 0x3e, 0xec, 0x10, 0xda, 0xab, 0x1f, 0x85, 0x56, 0xee, 0xa5, 0xc8, 0xe6, 0x93};
	...
	{
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].tx_write_clientservice_uuid = RSI_BLE_CLIENT_WRITE_SERVICE_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].tx_write_client_char_uuid = RSI_BLE_CLIENT_WRITE_CHAR_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].tx_wnr_client_service_uuid = RSI_BLE_CLIENT_WRITE_NO_RESP_SERVICE_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].tx_wnr_client_char_uuid = RSI_BLE_CLIENT_WRITE_NO_RESP_CHAR_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].rx_indi_client_service_uuid = RSI_BLE_CLIENT_INIDCATIONS_SERVICE_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].rx_indi_client_char_uuid = RSI_BLE_CLIENT_INIDCATIONS_CHAR_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].rx_notif_client_service_uuid = RSI_BLE_CLIENT_NOTIFICATIONS_SERVICE_UUID_M1;
		rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[i].rx_notif_client_char_uuid = RSI_BLE_CLIENT_NOTIFICATIONS_CHAR_UUID_M1;
	}
		...
		case RSI_APP_EVENT_ADV_REPORT: {
			...
			if(slave_task_instances < RSI_BLE_MAX_NBR_SLAVES)
			{
				...
				if((slave_conn_id < TOTAL_CONNECTIONS) && (slave_con_req_pending == 1))
				{
					...
					status = rsi_task_create((rsi_task_function_t)rsi_ble_task_on_conn, (uint8_t *)"ble_slave_task", RSI_BLE_APP_TASK_SIZE,  &rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[slave_task_instances], RSI_BLE_APP_TASK_PRIORITY, &ble_app_task_handle[slave_conn_id]);
					if(status != RSI_ERROR_NONE)
					{
						...
						memset(&rsi_ble_conn_info[ble_conn_id].rsi_app_adv_reports_to_app,
								0,
								sizeof(rsi_ble_event_adv_report_t));
						break;
					}
		...
		case RSI_BLE_CONN_EVENT: {
			...
			if(remote_device_role == MASTER_ROLE)
			{
				if(master_task_instances < RSI_BLE_MAX_NBR_MASTERS)
				{
					...
					{
						//! store the connection identifier in individual connection specific buffer
						rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[RSI_BLE_MAX_NBR_SLAVES+master_task_instances].conn_id = master_conn_id;

						//! create task for processing new master connection
						status = rsi_task_create((rsi_task_function_t)rsi_ble_task_on_conn, (uint8_t *)"ble_master_task", RSI_BLE_APP_TASK_SIZE, &rsi_parsed_conf.rsi_ble_config.rsi_ble_conn_config[RSI_BLE_MAX_NBR_SLAVES+master_task_instances] , RSI_BLE_APP_TASK_PRIORITY, &ble_app_task_handle[master_conn_id]);
						if(status != RSI_ERROR_NONE)
						{
				...
				if(master_task_instances < RSI_BLE_MAX_NBR_MASTERS)
				{
					...
					if(master_conn_id < TOTAL_CONNECTIONS)
					{
						...
						if(status != RSI_ERROR_NONE)
						{
							...
							//! remove device from local list
							rsi_remove_ble_conn_id(rsi_ble_conn_info[master_conn_id].remote_dev_addr);
							memset(&rsi_ble_conn_info[ble_conn_id].rsi_enhc_conn_status, 0, sizeof(rsi_ble_event_enhance_conn_status_t));
							break;
						}
