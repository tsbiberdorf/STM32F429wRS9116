...
#define RSI_MAX_NUM_CALLBACKS          17
...
typedef struct rsi_nwk_callback_s
{
  ...
  void   (*http_client_response_handler)(uint16_t status,const uint8_t *buffer, const uint16_t length, const uint32_t more_data, uint16_t status_code);
#else 
...
//! HTTP CLIENT PUT response notify handler pointer
  void   (*rsi_http_client_put_response_handler)(uint16_t status,uint8_t cmd_type, const uint8_t *buffer, uint16_t length, const uint8_t end_of_put_pkt);
  
  //! HTTP CLIENT POST data response notify handler pointer
#if RSI_HTTP_STATUS_INDICATION_EN
  void   (*rsi_http_client_post_data_response_handler)(uint16_t status,const uint8_t *buffer, const uint16_t length, const uint32_t more_data, uint16_t status_code);
#else 
