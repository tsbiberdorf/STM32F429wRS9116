...
int32_t rsi_http_client_async(uint8_t type, uint8_t flags, uint8_t *ip_address, uint16_t port, 
                            ...
                            uint8_t *user_name, uint8_t *password, uint8_t *post_data, uint32_t post_data_length,
#if RSI_HTTP_STATUS_INDICATION_EN
                            void(*callback)(uint16_t status, const uint8_t *buffer, const uint16_t length, const uint32_t moredata, uint16_t status_code)
#else 
               ...
               )
{
 ...
 { 
   ...
   {
     //! Register http client response notify call back handler
     rsi_wlan_cb_non_rom->nwk_callbacks.rsi_http_client_post_data_response_handler = rsi_http_post_data_response_handler;
   }
   else
   {
     //!Changing the common state to allow state
