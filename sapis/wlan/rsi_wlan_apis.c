...
/*==============================================*/
/**
 * @fn         int32_t rsi_wlan_scan_with_bitmap_options(int8_t *ssid, uint8_t chno, rsi_rsp_scan_t *result, uint32_t length,uint32_t scan_bitmap)
 ...
 * @param[in]  scan_bitmap , scan bimap options 
 * @return     int32_t 
 *             0  =  success
 *             <0 = failure 
 * @section description
...
/*==============================================*/
/**
 * @fn         int32_t rsi_wlan_scan_async_with_bitmap_options(int8_t *ssid, uint8_t chno,uint32_t bitmap, void (*callback)(uint16_t status, const uint8_t *buffer, const uint16_t length))
 * @brief      Scans for the access points 
 * @param[in]  ssid , ssid of the access point to scan 
 * @param[in]  chno , channel number of the access point 
      ...
      case RSI_WLAN_STATE_SCAN_DONE:
        {
          ...
          else if(((sec_type == RSI_WPA)||(sec_type == RSI_WPA2)||(sec_type == RSI_WPA_WPA2_MIXED)||(sec_type == RSI_WPA_PMK)||(sec_type == RSI_WPA2_PMK)) && (wlan_cb->opermode != RSI_WLAN_ENTERPRISE_CLIENT_MODE))
          {
            //! allocate command buffer  from wlan pool
            pkt = rsi_pkt_alloc(&wlan_cb->wlan_tx_pool);
 ...
 *              1 - Fast PSP
 *              2 - UAPSD 
 * @param[in]   listen interval.  \n
 * @param[out]  none
 * @return      command response status
 *
 * @section description
 * This function sets the different power save profile with listen interval based wakeup
 *
 */
    ...
    void (*wlan_wfd_connection_request_notify_handler)(uint16_t status, uint8_t *buffer, const uint32_t length))
{
  ...
  {
    rsi_wlan_cb_non_rom->callback_list.wlan_wfd_connection_request_notify_handler =  wlan_wfd_connection_request_notify_handler; 
  }
...
int32_t rsi_wlan_ping_async(uint8_t flags, uint8_t *ip_address, uint16_t size, void (*wlan_ping_response_handler)(uint16_t status, const uint8_t *buffer, const uint16_t length))
{
  ...
  {
    //! wait on wlan semaphore
    rsi_wait_on_wlan_semaphore(&rsi_driver_cb_non_rom->wlan_cmd_sem, RSI_PING_RESPONSE_WAIT_TIME); 
    //! get wlan/network command response status
    status =  rsi_wlan_get_status();
...
/*==============================================*/
/**
 * @fn                rsi_wlan_req_radio(uint8_t enable)
 * @brief             To register and de-register wlan radio 
 * @param[in]         uint8_t enable: To register ot de-register wlan radio  
...
/*==============================================*/
/**
 * @fn                rsi_wlan_radio_deinit()
 * @brief             To de-register wlan radio 
 * @param[in]         None  
 * @param[out]        RSI_SUCCESS 
 * @return
 *              Non zero - If fails
 ...
 *
 * @section description
 *  This function is used to de-register wlan radio. 
 *
 */

int32_t rsi_wlan_radio_deinit()
{
  int32_t          status = RSI_SUCCESS;
  //! req for wlan radio
